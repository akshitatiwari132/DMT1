...
-- prove proposition w/ lambda abstraction

def foo {P : Prop} : (P → False) → P → α :=
(
  fun pf =>
  (
...
| h  =>     -- assume: ¬(P ∧ Q), (P ∧ Q) → False; show (¬P ∨ ¬Q)
  (Or.inl
    (fun (p : P) =>
      (
        _-- I can't figure out the proof - I don't think it's provable? Will ask in class/OH.
      )
    )
  )
...
fun h => match h with
  | (Or.inl np)  => -- assume ¬P
    (
      ...
      (
        (np pq.left) -- we extract pq.lefr (P) then we apply np to it
      )
    )
  | (Or.inr nq) =>
    (
      fun pq =>
      (
        (nq pq.right) -- now we extract the right (Q) and apply nq, also showing false
      )
    )

  -- basically in either case we are showing ¬(P ∧ Q), so this is valid!
...
-- we combine using iff.intro
-- need 2 subproofs??

theorem new_one {P Q : Prop} :

-- little confused on how to get started here, will wait for class
-- def have to combine the work we did above (are those our subproofs?)
